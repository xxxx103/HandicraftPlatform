...
from extend import *
from pojo import *
from redis_cache import redis_cache, clear_all_product_cache
...
def detail():
    ...
    images = []
    if product.images:
        image_urls = convert_qiniu_url(product.images)
        images = [img for img in image_urls.split('@') if img.strip()]
...
def add_product():
    ...
    if csid is not None or csid != "":
        ...
        if categorySecond is None:
            ...
            # 更新用户身份为会员用户（仅当用户是普通用户时）
            if user.identity == 0:  # 如果是普通用户
                user.identity = 2    # 更新为会员用户(identity=2)
            # 管理员(identity=1)保持不变
...
def getClassify():
    ...
    # 构建基本查询
    query = Product.query.filter(Product.is_pass == 2).order_by(Product.pdate.desc())
    
    # 根据条件添加过滤
    if category is not None and category.id:
        for categorySecond_One in category.categoryseconds:
            csids.append(str(categorySecond_One.id))
        query = query.filter(Product.csid.in_(csids))
    elif csid is not None:
        query = query.filter(Product.csid == str(csid))
    
    # 添加商品名称搜索条件
    if pname:
        query = query.filter(Product.pname.like(f'%{pname}%'))
    
    # 执行分页
    page_all = query.paginate(
        page=int(pageNum),
        per_page=12,
        error_out=False
    )
    ...
    return render_template('classify.html', 
                         categorys=categorys, 
                         products=page_all.items, 
                         currentPage=page_all.page,
                         pages=page_all.pages, 
                         categorySecond=categorySecond, 
                         category_my=category,
                         search_name=pname)
