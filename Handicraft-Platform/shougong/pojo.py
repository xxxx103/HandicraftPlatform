...
class User(db.Model):
    ...
    def user_json(self):
        user_json = {}
        user_json["img_url"] = convert_qiniu_url(self.img_url)
...
class Product(db.Model):
    ...
    def product_json(self):
        ...
        if self.images:
            product_json["images"] = convert_qiniu_url(self.images.split('@')[0])
        else:
            product_json["images"] = None
        product_json["counts"] = self.counts
        product_json["old_price"] = self.old_price
        product_json["new_price"] = self.new_price
        if self.head_img:
            product_json["head_img"] = convert_qiniu_url(self.head_img)
        else:
            product_json["head_img"] = None
        return product_json
    ...
    def product_json1(self):
        ...
        product_json["pname"] = self.pname
        if self.images:
            product_json["images"] = convert_qiniu_url(self.images)
        else:
            product_json["images"] = None
        if self.head_img:
            product_json["head_img"] = convert_qiniu_url(self.head_img)
